  ...
  return (
    <main className="mx-auto flex min-h-[75vh] max-w-3xl flex-col justify-center px-4 py-16">
      <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-3 text-sm text-zinc-600">Google, Apple e Microsoft confermano la tua identità, ma HotelsDrop deve sapere se sei inserzionista o struttura ricettiva.</p>
        {error ? <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div> : null}
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          <button type="button" onClick={() => choose("inserzionista")} disabled={Boolean(loadingKind)} className="rounded-3xl border border-zinc-200 p-6 text-left transition hover:border-emerald-400 hover:bg-emerald-50 disabled:opacity-60">
            ...
            <span className="mt-5 inline-flex rounded-full bg-zinc-950 px-5 py-3 text-sm font-semibold text-white">{loadingKind === "inserzionista" ? "Creazione..." : "Continua come inserzionista"}</span>
          </button>
          <button type="button" onClick={() => choose("struttura")} disabled={Boolean(loadingKind)} className="rounded-3xl border border-zinc-200 p-6 text-left transition hover:border-emerald-400 hover:bg-emerald-50 disabled:opacity-60">
            ...
            <span className="mt-5 inline-flex rounded-full bg-zinc-950 px-5 py-3 text-sm font-semibold text-white">{loadingKind === "struttura" ? "Creazione..." : "Continua come struttura"}</span>
          </button>
        </div>
      </div>
    </main>
