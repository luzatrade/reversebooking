...
create policy "travel_requests_select_accessible"
on travel_requests
for select
to authenticated
using (public.can_access_travel_request(id));

create policy "offers_select_accessible"
on offers
for select
to authenticated
using (public.can_access_offer(id));

create policy "offers_update_accessible"
on offers
for update
to authenticated
using (public.can_access_offer(id))
...
create policy "offer_messages_select_accepted_participants"
on offer_messages
for select
to authenticated
using (public.can_access_accepted_offer(offer_id));

create policy "offer_messages_insert_accepted_participants"
on offer_messages
for insert
to authenticated
