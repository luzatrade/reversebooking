...
async function downloadImage(imageUrl, referer = null) {
  const headers = {
    "User-Agent": "Mozilla/5.0 (compatible; HotelsDropBot/1.0)",
    Accept: "image/*,*/*",
  };
  ...
  if (!res.ok) throw new Error(`Download fallito (${res.status}) ${imageUrl}`);
  const buf = Buffer.from(await res.arrayBuffer());
  if (buf.length < 500) throw new Error(`Immagine troppo piccola: ${imageUrl}`);
...
async function importHotel(hotel, usedSlugs) {
  ...
  const photoUrls = PHOTO_MAP[slug] ?? [];
