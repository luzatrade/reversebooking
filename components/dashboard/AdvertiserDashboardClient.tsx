  ...
  return (
    <main className={`${dashboardSurfaces.page} ${dashboardSurfaces.shell}`}>
      ...
      <div className={`${dashboardSurfaces.headerPanel} flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between`}>
        <div>
          <p className={dashboardSurfaces.areaLabel}>Area inserzionista</p>
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-[#0c3d66]">Dashboard inserzionista</h1>
          ...
          {activeRequest ? (
            <p className="mt-2 text-sm text-zinc-500">
              Richiesta selezionata: {activeRequest.city_name} · {activeRequest.preferred_area}
            </p>
          ) : null}
        </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-5">
        <StatCard tone="blue" label="Annunci attivi" value={loading ? "..." : String(activeCount)} description="Richieste pubblicate e ancora aperte" />
        <StatCard tone="cream" label="Offerte ricevute" value={loading ? "..." : String(offers.length)} description="Proposte sulla richiesta selezionata" />
        <StatCard tone="accepted" label="Offerte accettate" value={loading ? "..." : String(acceptedOffers.length)} description="Soggiorni confermati" />
        <StatCard tone="white" label="Strutture in zona" value={loading ? "..." : String(hotels.length)} description="Filtrate per città e tipologia" />
      ...
      <section className={`${dashboardSurfaces.acceptedPanel} mt-8`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-orange-950">Offerte accettate / soggiorni confermati</h2>
            <p className="text-sm text-orange-900/85">Qui trovi le offerte già accettate. Per scrivere usa la chat flottante in basso a destra.</p>
          </div>
        </div>
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  <p className="font-semibold text-zinc-950">
                    {offer.offer_code ?? "OF------"} · {offer.hotel_accounts?.property_name ?? "Struttura"}
                  </p>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_380px]">
        <section className={dashboardSurfaces.panelCream}>
          <div className="flex items-center gap-3">
            <Building2 className={dashboardSurfaces.sectionIcon} />
            <h2 className={dashboardSurfaces.sectionTitle}>Strutture ricettive della zona</h2>
          </div>
          <p className={dashboardSurfaces.sectionSubtitle}>Elenco informativo filtrato da città e tipologia struttura.</p>
          <div className="mt-5 space-y-3">
            {loading ? <p className="text-sm text-zinc-500">Caricamento strutture...</p> : null}
            {!loading && hotels.length === 0 ? (
              <div className={dashboardSurfaces.emptyDashed}>Nessuna struttura compatibile trovata nella zona della richiesta.</div>
            ) : null}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      {hotel.points_of_interest?.length ? (
                        <p className="mt-2 text-xs text-zinc-500">Vicino a: {hotel.points_of_interest.join(", ")}</p>
                      ) : null}
        ...
        <aside className={dashboardSurfaces.panelBlue}>
          <div className="flex items-center gap-3">
            <Bell className={dashboardSurfaces.sectionIcon} />
            <h2 className={dashboardSurfaces.sectionTitle}>Campanella offerte</h2>
          </div>
          <p className={dashboardSurfaces.sectionSubtitle}>Lista delle strutture che hanno fatto una proposta.</p>
          <div className="mt-5 space-y-3">
            {loading ? <p className="text-sm text-zinc-500">Caricamento offerte...</p> : null}
            {!loading && offers.length === 0 ? (
              <div className={dashboardSurfaces.emptyDashed}>Nessuna offerta ricevuta al momento.</div>
            ) : null}
            {offers.map((offer, index) => (
              <article key={offer.id} className={index % 2 === 0 ? dashboardSurfaces.cardBlue : dashboardSurfaces.cardWhite}>
                ...
                <p className="text-sm text-zinc-600">
                  ...
                  {formatCurrency(Number(offer.total_price))}
                </p>
                <p className="mt-1 text-xs text-zinc-500">
                  {mealPlanLabels[offer.meal_plan_included]} · {offer.status} · {formatDate(offer.created_at)}
                </p>
      ...
      {loading && !profile ? <p className="mt-4 text-xs text-zinc-400">Caricamento profilo inserzionista...</p> : null}
    </main>
  );
}
