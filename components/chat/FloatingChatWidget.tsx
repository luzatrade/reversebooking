      ...
      {open ? (
        <div className="mb-3 w-[calc(100vw-2rem)] max-w-[calc(100vw-2rem)] overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900 sm:w-[620px] sm:max-w-[620px] lg:w-[700px] lg:max-w-[700px]">
          <div className="flex items-center justify-between border-b border-zinc-200 p-4 dark:border-zinc-800">
            <div className="min-w-0 pr-3"><p className="text-sm font-semibold">Chat</p><p className="text-xs text-zinc-500">Offerte accettate · aggiornata automaticamente</p></div>
          ...
          <div className="flex max-h-[75vh] flex-col sm:max-h-[560px] sm:grid sm:grid-cols-[185px_minmax(0,1fr)]">
            <div className="flex gap-2 overflow-x-auto border-b border-zinc-200 p-2 dark:border-zinc-800 sm:block sm:h-[500px] sm:overflow-y-auto sm:border-b-0 sm:border-r">
              ...
              {offers.map((offer) => {
                ...
                return <button key={offer.id} type="button" onClick={() => setActiveOfferId(offer.id)} className={`min-w-[155px] rounded-2xl p-3 text-left text-xs sm:mb-1 sm:w-full sm:min-w-0 ${offer.id === activeOfferId ? "bg-emerald-50 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}`}><span className="block break-words font-semibold leading-tight">{offerHotel?.property_name ?? "Struttura"}</span><span className="mt-1 block break-words leading-tight text-zinc-500">{offerRequest?.city_name ?? "Chat"}</span></button>;
              })}
            </div>
            <div className="flex h-[62vh] min-h-[360px] min-w-0 flex-col sm:h-[500px] sm:min-h-0">
              ...
              <form onSubmit={sendMessage} className="flex gap-2 border-t border-zinc-200 p-3 dark:border-zinc-800"><input value={body} onChange={(event) => setBody(event.target.value)} placeholder="Scrivi..." className="min-w-0 w-full rounded-full border border-zinc-300 bg-white px-4 py-3 text-sm dark:border-zinc-700 dark:bg-zinc-950 sm:py-2 sm:text-xs" /><button disabled={sending || !body.trim() || !activeOffer} type="submit" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-zinc-950 text-white disabled:opacity-50 dark:bg-white dark:text-zinc-950 sm:h-9 sm:w-9" aria-label="Invia messaggio"><Send className="h-4 w-4" /></button></form>
            </div>
          </div>
        </div>
      ) : null}
      <button onClick={() => setOpen((value) => !value)} className="relative flex h-14 w-14 items-center justify-center rounded-full bg-emerald-700 text-white shadow-2xl" aria-label="Apri chat"><MessageCircle className="h-6 w-6" />{unreadCount > 0 ? <span className="absolute -right-1 -top-1 flex h-6 min-w-6 items-center justify-center rounded-full bg-red-600 px-2 text-xs font-bold text-white">{unreadCount > 99 ? "99+" : unreadCount}</span> : null}</button>
    </div>
  );
}
