...
export function FloatingChatWidget() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex max-w-[calc(100vw-1.5rem)] flex-col items-end">
      {open ? (
        <div className="mb-3 w-[min(100vw-1.5rem,920px)] overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-zinc-200 p-4">
            ...
            <button onClick={() => setOpen(false)} className="rounded-full p-2 hover:bg-zinc-100" aria-label="Chiudi chat">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="flex max-h-[min(82vh,720px)] flex-col lg:grid lg:grid-cols-[minmax(220px,280px)_minmax(0,1fr)]">
            <div className="max-h-[220px] overflow-x-auto border-b border-zinc-200 p-2 lg:max-h-none lg:overflow-y-auto lg:border-b-0 lg:border-r">
              ...
              {offers.map((offer) => {
                ...
                return (
                  <button
                    ...
                    className={`mb-1 w-full min-w-[200px] rounded-2xl p-3 text-left text-xs lg:min-w-0 ${ offer.id === activeOfferId
                        ? "bg-emerald-50 text-emerald-800"
                        : "hover:bg-zinc-100"
                   }`}
                  >
                    <span className="block break-words font-semibold leading-tight">{counterpartName(offer, role)}</span>
                    <span className="mt-1 inline-flex rounded-full bg-white px-2 py-0.5 text-[11px] font-bold text-zinc-700">
                      {requestCode(offer)}
                    </span>
            ...
            <div className="flex min-h-[420px] min-w-0 flex-col lg:min-h-[560px]">
              <div className="border-b border-zinc-200 p-3">
                ...
                <p className="mt-1 break-words text-xs leading-tight text-zinc-500">
                  <span className="font-bold text-zinc-700">{requestCode(activeOffer)}</span>
                ...
                {!chatStillOpen && activeRequest ? (
                  <p className="mt-2 text-xs text-amber-700">Chat chiusa: sono passate più di 24 ore dal check-in previsto.</p>
                ) : null}
              </div>
                ...
                {messages.map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`max-w-[92%] rounded-2xl p-3 text-sm leading-relaxed lg:max-w-[75%] ${ mine ? "bg-zinc-950 text-white" : "bg-zinc-100"
                       }`}
                      >
              ...
              <form onSubmit={sendMessage} className="flex gap-2 border-t border-zinc-200 p-3">
                <input
                  ...
                  className="min-w-0 w-full rounded-full border border-zinc-300 bg-white px-4 py-3 text-sm disabled:opacity-50"
                />
                <button
                  disabled={sending || !body.trim() || !activeOffer || !chatStillOpen}
                  type="submit"
                  className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-zinc-950 text-white disabled:opacity-50"
