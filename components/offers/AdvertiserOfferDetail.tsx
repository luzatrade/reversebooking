...
export function AdvertiserOfferDetail() {
  ...
  if (loading) {
    return <div className="rounded-3xl border p-6 text-sm text-zinc-500">Caricamento offerta...</div>;
  }
        ...
        <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-zinc-200 p-5 dark:border-zinc-800">
              <h2 className="text-lg font-semibold">Struttura</h2>
              <p className="mt-2 font-medium">{offer.hotel_accounts?.property_name ?? "Struttura"}</p>
              <p className="text-sm text-zinc-500">
                {offer.hotel_accounts?.structure_type ? structureTypeLabels[offer.hotel_accounts.structure_type] : "Struttura"} · {offer.hotel_accounts?.city_name}
              </p>
              {offer.hotel_accounts?.specific_area ? <p className="mt-1 text-sm text-zinc-500">Zona: {offer.hotel_accounts.specific_area}</p> : null}
              ...
              {offer.hotel_accounts?.id ? (
                <Link href={`/hotel/${offer.hotel_accounts.id}`} className="mt-4 inline-flex rounded-full border px-4 py-2 text-sm font-semibold">
                  Visualizza profilo hotel
                </Link>
              ) : null}
            </div>

            <div className="rounded-2xl border border-zinc-200 p-5 dark:border-zinc-800">
              <h2 className="text-lg font-semibold">Dettagli proposta</h2>
              <p className="mt-2 text-sm"><strong>Trattamento incluso:</strong> {mealPlanLabels[offer.meal_plan_included]}</p>
              <p className="mt-3 text-sm text-zinc-600 dark:text-zinc-400">{offer.description}</p>
              {offer.conditions ? <p className="mt-3 text-sm text-zinc-500"><strong>Condizioni:</strong> {offer.conditions}</p> : null}
            </div>
          </div>

          <div className="mt-6 rounded-2xl border border-zinc-200 p-5 text-sm dark:border-zinc-800">
            <h2 className="text-lg font-semibold">Richiesta collegata</h2>
          ...
          {offer.status === "pending" ? (
            <div className="mt-6 flex flex-wrap gap-3">
              <button disabled={saving} onClick={() => updateOfferStatus("accepted")} className="inline-flex items-center gap-2 rounded-full bg-emerald-700 px-6 py-3 text-sm font-semibold text-white disabled:opacity-60">
                <CheckCircle2 className="h-4 w-4" /> Accetta offerta
              </button>
              <button disabled={saving} onClick={() => updateOfferStatus("rejected")} className="inline-flex items-center gap-2 rounded-full border border-red-200 px-6 py-3 text-sm font-semibold text-red-700 disabled:opacity-60">
