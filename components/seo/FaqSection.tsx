...
function FaqList({ items, compact }: { items: FaqItem[]; compact: boolean }) {
  return (
...
export function FaqSection({ items, sections, title, id = "faq", compact = false }: Props) {
    ...
    <section aria-labelledby={id} className="scroll-mt-24">
      ...
      {hasSections ? (
        <div className={`mt-4 space-y-8 sm:mt-6 ${compact ? "max-w-2xl" : "max-w-3xl"}`}>
          {sections!.map((section) => (
            <div key={section.title}>
              ...
              <div className="mt-3">
                <FaqList items={section.items} compact={compact} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`mt-4 sm:mt-5 ${compact ? "max-w-2xl" : "max-w-3xl"}`}>
          <FaqList items={items ?? []} compact={compact} />
        </div>
      )}
    </section>
  );
