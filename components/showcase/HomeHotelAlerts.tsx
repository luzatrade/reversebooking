        ...
        <button type="button" onClick={openDashboardAndMarkRead} className="flex items-start gap-3 text-left">
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-sm">
              {state.requestsInArea > 0 ? <span className="rounded-full bg-white px-3 py-1 font-medium shadow-sm dark:bg-zinc-900">{state.requestsInArea} nuove richieste nella tua zona</span> : null}
              {state.acceptedOffers > 0 ? <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-3 py-1 font-medium text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200"><CheckCircle2 className="h-3.5 w-3.5" /> {state.acceptedOffers} offerta accettata{state.acceptedOffers > 1 ? "e" : ""}</span> : null}
              {state.rejectedOffers > 0 ? <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-3 py-1 font-medium text-red-800 dark:bg-red-950 dark:text-red-200"><XCircle className="h-3.5 w-3.5" /> {state.rejectedOffers} offerta rifiutata{state.rejectedOffers > 1 ? "e" : ""}</span> : null}
            </div>
          </div>
        </button>
