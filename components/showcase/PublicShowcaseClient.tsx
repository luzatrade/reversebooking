...
export function PublicShowcaseClient() {
  ...
  const [requestCheckInFilter, setRequestCheckInFilter] = useState("");
  const [requestCheckOutFilter, setRequestCheckOutFilter] = useState("");
  const [requestGuestsFilter, setRequestGuestsFilter] = useState("");
  const [requestRoomsFilter, setRequestRoomsFilter] = useState("");
  const [compactRequests, setCompactRequests] = useState(false);
  ...
  function clearRequestFilters() { setRequestCityFilter(""); setRequestCheckInFilter(""); setRequestCheckOutFilter(""); setRequestGuestsFilter(""); setRequestRoomsFilter(""); }
  ...
  const filteredRequests = useMemo(() => {
    const text = requestCityFilter.trim().toLowerCase();
    const guests = Number(requestGuestsFilter || 0);
    const rooms = Number(requestRoomsFilter || 0);
    return requests.filter((request) => {
      if (text && !`${request.city_name} ${request.preferred_area} ${request.notes ?? ""}`.toLowerCase().includes(text)) return false;
      if (requestCheckInFilter && request.check_in < requestCheckInFilter) return false;
      if (requestCheckOutFilter && request.check_out > requestCheckOutFilter) return false;
      if (guests > 0 && request.guests_count < guests) return false;
      if (rooms > 0 && request.rooms_count < rooms) return false;
      return true;
    });
  }, [requests, requestCityFilter, requestCheckInFilter, requestCheckOutFilter, requestGuestsFilter, requestRoomsFilter]);
  ...
  return <main className="min-h-screen bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-white">
    <header className="border-b border-zinc-200 bg-white/90 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/80">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-wide text-emerald-700">Reverse Booking</p><h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Home richieste e strutture</h1><p className="mt-2 max-w-2xl text-sm text-zinc-600 dark:text-zinc-400">Smetti di cercare l'hotel perfetto. Pubblica la tua richiesta e lascia che gli hotel facciano a gara per averti.</p></div><div className="flex flex-wrap gap-3">{viewer.userId ? <Link href={dashboardHref(viewer)} className="inline-flex items-center gap-2 rounded-full border border-zinc-300 px-5 py-3 text-sm font-semibold dark:border-zinc-700"><LayoutDashboard className="h-4 w-4" /> Vai alla dashboard{viewer.role === "advertiser" && advertiserOfferCount > 0 ? <span className="ml-1 rounded-full bg-red-600 px-2 py-0.5 text-xs text-white">{advertiserOfferCount}</span> : null}</Link> : <Link href="/login" className="rounded-full border border-zinc-300 px-5 py-3 text-sm font-semibold dark:border-zinc-700">Login</Link>}</div></div>
        <div className="rounded-[1.5rem] border border-zinc-200 bg-zinc-50 p-3 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_.7fr_.7fr_.7fr]"><label className="text-xs font-semibold text-zinc-500">Città<input value={citySearch} onChange={(event) => setCitySearch(event.target.value)} list="home-city-suggestions" placeholder="Dove vuoi andare?" className="mt-2 h-13 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /><datalist id="home-city-suggestions">{citySuggestions.map((city) => <option key={city.city_id} value={city.label} />)}</datalist></label><AppDatePicker label="Check-in" value={checkInSearch} onChange={setCheckInSearch} minDate={today} /><AppDatePicker label="Check-out" value={checkOutSearch} onChange={setCheckOutSearch} minDate={checkInSearch || today} /><label className="text-xs font-semibold text-zinc-500">Adulti<input type="number" min={0} value={adultsSearch} onChange={(event) => setAdultsSearch(event.target.value)} placeholder="2" className="mt-2 h-13 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label><label className="text-xs font-semibold text-zinc-500">Bambini<input type="number" min={0} value={childrenSearch} onChange={(event) => setChildrenSearch(event.target.value)} placeholder="0" className="mt-2 h-13 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label><label className="text-xs font-semibold text-zinc-500">Camere<input type="number" min={1} value={roomsSearch} onChange={(event) => setRoomsSearch(event.target.value)} placeholder="1" className="mt-2 h-13 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label></div>
          <div className="mt-3 grid gap-3 md:grid-cols-[minmax(160px,320px)_minmax(140px,210px)_112px_auto]"><label className="text-xs font-semibold text-zinc-500">Zona<input value={areaSearch} onChange={(event) => setAreaSearch(event.target.value)} placeholder="Centro, fiera, mare..." className="mt-2 h-11 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label><label className="text-xs font-semibold text-zinc-500">Budget per camera<input type="number" min={1} value={budgetSearch} onChange={(event) => setBudgetSearch(event.target.value)} placeholder="€" className="mt-2 h-11 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label><div className="flex items-end"><button type="button" onClick={() => setFiltersOpen((value) => !value)} className="inline-flex h-11 w-28 items-center justify-center gap-2 rounded-2xl border border-white/70 bg-white px-3 text-sm font-semibold text-[#0f4c81] shadow-sm"><Filter className="h-4 w-4" /> Filtri{selectedFilterCount ? ` (${selectedFilterCount})` : ""}</button></div><div className="flex items-end"><Link href={requestDraftHref} className="inline-flex h-11 items-center whitespace-nowrap rounded-2xl bg-white px-5 text-sm font-semibold text-[#0f4c81] shadow-sm">Crea richiesta</Link></div></div>
          {filtersOpen ? <div className="mt-3 max-w-3xl rounded-3xl border border-white/60 bg-white/95 p-3 shadow-sm"><p className="text-sm font-semibold text-[#0f4c81]">Filtri e preferenze richiesta</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{filterOptions.map((filter) => <label key={filter.key} className="flex cursor-pointer items-center gap-2 rounded-2xl border border-zinc-200 px-3 py-2 text-sm font-semibold text-zinc-700"><input type="checkbox" checked={Boolean(selectedFilters[filter.key])} onChange={() => toggleFilter(filter.key)} />{filter.label}</label>)}</div></div> : null}
        </div>
      </div>
    </header>
    <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[360px_1fr] lg:px-8">
      ...
      <section className="space-y-4"><div className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.25fr_1fr_1fr_.65fr_.65fr_auto]"><label className="text-xs font-semibold text-zinc-500">Città<input value={requestCityFilter} onChange={(event) => setRequestCityFilter(event.target.value)} placeholder="Città o zona" className="mt-2 h-11 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label><AppDatePicker label="Check-in" value={requestCheckInFilter} onChange={setRequestCheckInFilter} minDate={today} size="sm" /><AppDatePicker label="Check-out" value={requestCheckOutFilter} onChange={setRequestCheckOutFilter} minDate={requestCheckInFilter || today} size="sm" /><label className="text-xs font-semibold text-zinc-500">Ospiti<input type="number" min={0} value={requestGuestsFilter} onChange={(event) => setRequestGuestsFilter(event.target.value)} placeholder="Min" className="mt-2 h-11 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label><label className="text-xs font-semibold text-zinc-500">Camere<input type="number" min={0} value={requestRoomsFilter} onChange={(event) => setRequestRoomsFilter(event.target.value)} placeholder="Min" className="mt-2 h-11 w-full rounded-2xl border border-zinc-300 bg-white px-4 text-sm text-zinc-950 outline-none dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" /></label><div className="flex items-end"><button type="button" onClick={() => setCompactRequests((value) => !value)} className={`h-11 whitespace-nowrap rounded-2xl border px-4 text-xs font-semibold ${compactRequests ? "bg-[#0f4c81] text-white" : "bg-white text-[#0f4c81]"}`}>Vista compatta</button></div></div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-zinc-500"><span>{filteredRequests.length} richieste trovate</span>{requestCityFilter || requestCheckInFilter || requestCheckOutFilter || requestGuestsFilter || requestRoomsFilter ? <button type="button" onClick={clearRequestFilters} className="font-semibold text-[#0f4c81]">Pulisci filtri</button> : null}</div></div>{error ? <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div> : null}{loading ? <div className="rounded-3xl border border-zinc-200 bg-white p-6 text-sm text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900">Caricamento home...</div> : null}{!loading && filteredRequests.length === 0 ? <div className="rounded-3xl border border-dashed border-zinc-300 bg-white p-8 text-center dark:border-zinc-800 dark:bg-zinc-900"><p className="font-semibold">Nessuna richiesta trovata</p><p className="mt-2 text-sm text-zinc-500">Modifica o pulisci i filtri per vedere più annunci.</p>{viewer.role !== "hotel" ? <Link href={requestDraftHref} className="mt-4 inline-flex rounded-full bg-emerald-400 px-5 py-3 text-sm font-semibold text-emerald-950">Crea la tua richiesta</Link> : null}</div> : null}{filteredRequests.map((request) => { const filters = activeFilterLabels(request.preference_filters).slice(0, compactRequests ? 3 : 5); const hasOffer = offeredRequestIds.has(request.id); return <article key={request.id} className={`rounded-3xl border border-zinc-200 bg-white shadow-sm transition hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 ${compactRequests ? "p-4" : "p-5"}`}><div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">{badgeFor(request)}</span><span className="rounded-full bg-zinc-100 px-3 py-1 text-xs font-semibold dark:bg-zinc-800">{request.city_name}</span><span className="rounded-full bg-zinc-100 px-3 py-1 text-xs dark:bg-zinc-800">{mealPlanLabels[request.meal_plan]}</span></div><p className="mt-3 text-sm font-semibold text-emerald-700">Pubblicato da {advertiserName(request)}</p><h3 className={`${compactRequests ? "mt-1 text-lg" : "mt-1 text-xl"} font-semibold`}>Richiesta soggiorno a {request.city_name}</h3><p className="mt-1 text-sm text-zinc-500">Zona preferita: {request.preferred_area}</p><div className={`${compactRequests ? "mt-3" : "mt-4"} grid gap-3 text-sm text-zinc-600 sm:grid-cols-3 dark:text-zinc-400`}><p className="inline-flex items-center gap-2"><CalendarDays className="h-4 w-4" /> {formatDate(request.check_in)} → {formatDate(request.check_out)}</p><p className="inline-flex items-center gap-2"><Users className="h-4 w-4" /> {request.guests_count} ospiti · {request.rooms_count} camere</p><p className="inline-flex items-center gap-2"><Euro className="h-4 w-4" /> Per camera {formatCurrency(Number(request.budget))}</p></div>{!compactRequests ? <div className="mt-3 grid gap-2 rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-sm text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950/60 dark:text-zinc-400 sm:grid-cols-2"><p><strong>Budget per camera:</strong> {formatCurrency(Number(request.budget))}</p><p><strong>Totale indicativo:</strong> {formatCurrency(totalBudget(request))}</p></div> : null}{filters.length ? <div className="mt-4 flex flex-wrap gap-2">{filters.map((filter) => <span key={filter} className="rounded-full border border-zinc-200 px-3 py-1 text-xs font-semibold text-zinc-600 dark:border-zinc-700 dark:text-zinc-300">{filter}</span>)}</div> : null}{request.notes && !compactRequests ? <p className="mt-4 line-clamp-2 text-sm text-zinc-600 dark:text-zinc-400">{request.notes}</p> : null}</div><div className="flex shrink-0 flex-col gap-2 md:w-56">{isHotel ? hasOffer ? <div className="rounded-full border border-emerald-200 px-4 py-3 text-center text-sm font-semibold text-emerald-700 dark:border-emerald-900 dark:text-emerald-300">Offerta già inviata</div> : <Link href={`/struttura/annunci/${request.id}`} className="rounded-full bg-emerald-400 px-4 py-3 text-center text-sm font-semibold text-emerald-950">Fai un’offerta</Link> : <Link href={requestDraftHref} className="rounded-full bg-emerald-400 px-4 py-3 text-center text-sm font-semibold text-emerald-950">Crea la tua richiesta</Link>}<button type="button" className="rounded-full border border-zinc-300 px-4 py-3 text-sm font-semibold dark:border-zinc-700">Vedi dettagli</button>{!compactRequests ? <p className="text-center text-[11px] text-zinc-400">Contatti protetti fino all’accettazione dell’offerta.</p> : null}</div></div></article>; })}</section>
    </div>
  </main>;
}
