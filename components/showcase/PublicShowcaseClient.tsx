...
export function PublicShowcaseClient() {
  ...
  const citySearchValue = citySearch.trim().toLowerCase();
  const visibleHotels = useMemo(() => hotels.filter((hotel) => !citySearchValue || `${hotel.city_name} ${hotel.specific_area ?? ""} ${hotel.property_name}`.toLowerCase().includes(citySearchValue)).slice(0, 8), [hotels, citySearchValue]);
  const filteredRequests = useMemo(() => requests.filter((request) => !citySearchValue || `${request.city_name} ${request.preferred_area} ${request.notes ?? ""}`.toLowerCase().includes(citySearchValue)), [requests, citySearchValue]);
